...
func (d *DB) DeleteMemo(ctx context.Context, delete *store.DeleteMemo) error {
	...
	if _, err := tx.ExecContext(ctx, "DELETE FROM poll_vote WHERE memo_id = "+placeholder(1), delete.ID); err != nil {
		return errors.Wrap(err, "failed to delete memo poll votes")
	}
	if _, err := tx.ExecContext(ctx, "DELETE FROM poll WHERE memo_id = "+placeholder(1), delete.ID); err != nil {
